...
8. **Telegram pairing codes persist** → When `/start` in Telegram generates a new pairing code, previous pending requests remain in the queue — they are not invalidated. Always check `openclaw pairing list` first to see existing pending requests before generating new codes. Use `openclaw pairing approve <code>` directly via CLI rather than relying solely on the nodes tool, which may not properly access pending requests.
